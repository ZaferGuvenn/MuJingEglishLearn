 ...
 * @param setProgressText ilerleme metnini ayarla
 */
...
fun parseDocument(
    ...
    try{
        if (extension == "pdf") {
            ...
            text = file.readText()
            // Windows metin dosyalarından BOM'u kaldır
    ...
    // Kelime -> Cümle eşlemesi, kelimelerin belgedeki konumlarını kaydetmek için kullanılır
    ...
    // Kelime türü etiketleme modelini yükle
    ...
    sentences.forEach { sentence ->
        ...
        wordList.forEach { word ->
            ...
            // Kod parçacıklarındaki anahtar kelimeler arasında . veya _ sembollerini kullanın
    ...
    val filterList = listOf(
        ".", "!", "?", ";", ":",  ")",  "}",  "]", "-", "—",
        "'", "`", "~", "@", "#", "$", "%", "^", "&", "*", "+", "=", "|",
        "/", ">", ",", "，", "。", "、", "；", "：", "？", "！", // TODO: Çince noktalama işaretleri kalmalı mı?
        "）","【", "】", "｛", "…",  "》", "”", "’" // TODO: Çince noktalama işaretleri kalmalı mı?
    )
    ...
    if(tokenList.size > sentenceLength){
        ...
        if(index != -1){
           ...
           val strIndex = formatSentence.indexOf(word)
            ...
            // strIndex merkez olacak şekilde, başlangıcı belirlemek için geriye doğru sentenceLength/2 boşluk bulun, başlangıç konumu 0'dan küçükse 0'dan başlayın
            // Bitişi belirlemek için ileriye doğru sentenceLength/2 boşluk bulun, bitiş konumu sentences uzunluğundan büyükse bitiş olarak sentences uzunluğunu kullanın
            var start = strIndex
            var end = strIndex
            var spaceCount = 0
...
/**
 * Kelimeleri ve ifadeleri bölmek için kelime türü etiketleme ve öbekleme kullanın
 */
fun tokenizeAndChunkText(
    ...
    // Kelime ayırma yapılıyor
    val tokens = tokenizer.tokenize(text)
    // Kelime türü etiketleme yapılıyor
    ...
    for (chunk in chunks) {
        ...
        word = word.trim()
        // Kelimenin ilk karakteri bir noktalama işaretiyse, noktalama işaretini atla
        if (word.length>1 && filterList.contains(word[0].toString())) {
            logger.info("$word başlangıç noktalama işaretini atla") // "丢弃开始标点" -> "başlangıç noktalama işaretini atla"
            word = word.substring(1)
        }
        // Kelimenin son karakteri bir noktalama işaretiyse, noktalama işaretini atla
        if (word.length>1 && filterList.contains(word[word.length - 1].toString())) {
            logger.info("$word bitiş noktalama işaretini atla") // "丢弃结束标点" -> "bitiş noktalama işaretini atla"
            word = word.substring(0, word.length - 1)
        }
        // Kelimenin başındaki ve sonundaki boşlukları tekrar atla
        word = word.trim()
        wordList.add(word)
    }
    ...
    try {
        ...
        for (caption in captionList) {
            ...
            val dataCaption = Caption(
                // getTime(format) tarafından döndürülen süre oynatılamıyor
...
fun parseASS(
    ...
    setProgressText("Altyazı dönüştürme başlatılıyor")
...
fun parseMKV(
    ...
    try {
        ...
        if (!reader.readHeader()) {
            println("Bu bir mkv dosyası değil!")
            return listOf()
        }
...
fun batchReadMKV(
    ...
    for (file in selectedFileList) {
        ...
        try {
            ...
            if (!reader.readHeader()) {
                logger.error("Bu video standart bir MKV dosyası değil")
            ...
            var trackID = -1
            // Parça adı ve parça ID eşlemesi, birden fazla İngilizce altyazı olabilir
            ...
            if (exception.message != null) {
                ...
                errorMessage[file] = "Altyazı formatı desteklenmiyor"
                logger.error("${file.nameWithoutExtension} altyazı formatı desteklenmiyor")
            }
            ...
            updateTaskState(Pair(file, false))
            errorMessage[file] = "Null işaretçi istisnası"
            logger.error("${file.nameWithoutExtension} null işaretçi istisnası\n ${ exception.printStackTrace()}")
            setCurrentTask(null)
            continue
        } finally {
            try {
...
/** Bazı altyazılar sabit bir konumda değildir, bunun yerine karakterlerin yanına yerleştirilir, bu işlev konum bilgilerini siler */
